function validateBMI(req, res, next) {
  ...
  // This checks if the request has a body or query
  if (!source) {
  ...
  if (isNaN(weight) || isNaN(height)) {
    return res
      .status(400)
      .json({ error: "Weight and height inputted values must be number." });
  }
  ...
  if (parseFloat(weight) <= 0 || parseFloat(height) <= 0) {
    return res
      .status(400)
      .json({ error: "Weight and height inputted values must be positive." });
  }

  if (unit !== "metric" && unit !== "imperial") {
    return res
      .status(400)
      .json({ error: "Unit must be either 'metric' or 'imperial'." });
  }
